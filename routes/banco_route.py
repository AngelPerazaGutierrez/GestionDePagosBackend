...
#
#
# @banco.put(f"/editar-banco/{banco_id}/{banco_nit}", response_model=GestionDePagosBackend.config.schemas.banco_schema.Banco)
# async def editar_banco(banco_id, banco_nit: int, banco: GestionDePagosBackend.config.schemas.banco_schema.Banco,
#                     db: Session = Depends(get_db)):
#         db_banco = db.query(GestionDePagosBackend.models.banco_model.Banco).filter(
#             GestionDePagosBackend.models.banco_model.Banco.nit == banco_nit).first()
#
#         if not db_banco:
#             raise HTTPException(status_code=404, detail="Banco no encontrado")
#
#         db.query(GestionDePagosBackend.models.banco_model.Banco).filter(GestionDePagosBackend.models.banco_model.Banco.nit == banco_nit).update({
#             "nit": banco.nit,
